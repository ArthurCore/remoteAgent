...
|---|---|---|
| Last 30 scheduled backup jobs are 30/30 successful | Record `backup_history_started_at`; then obtain at least 7 consecutive scheduled production-shaped staging backup/PITR points with no failed, missed, disabled, or unclassified run, plus the two isolated restores below. Production backup monitoring is enabled and its test alert is received before launch. Manual/accelerated backups do not count toward 7 or 30. | As soon as 30 scheduled production jobs exist, the rolling last-30 scheduled result must be 30/30 for every later release and continuing Agent-phase eligibility. The counter is not reset by a schedule/configuration change. |
| Monthly restore history | Exactly two isolated restores using the candidate schema and procured plan, from two distinct scheduled backup/recovery IDs on two different UTC calendar dates: one uses the newest available recovery point and one uses an independently selected older PITR point. At least one is the full timed 1,000,000-message/10-GiB drill; both use clean disposable environments and pass RPO/RTO, row/checksum, tenant-isolation, search/file, attachment, cursor-resume, and new-message checks. | Monthly after launch and on every schema-migration release. Restore evidence expires after 30 days. While fewer than 30 scheduled production jobs exist, every release candidate also restores the newest available backup successfully. |
...
The release manifest records `bootstrap=true`, `backup_history_started_at`, history observed/success/required/remaining counts, both backup/recovery IDs and UTC dates, which restore used the full fixture, alert-drill result, timestamps, and artifact links. The bootstrap must be completed before `M1-OPS` passes and therefore before Agent-phase entry. If any bootstrap run fails, fix the cause and repeat the affected sequence; do not omit failed evidence or count ad hoc manual snapshots as scheduled jobs.
