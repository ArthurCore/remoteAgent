...
After re-reading the patched S7 card, current integration test, frozen migration artifacts, pinned Drizzle implementation, and cumulative checker following the canonical boundary RED, the corrected S7 contract is feasible, complete, and internally consistent. The forbidden `packages/db/src/migrate` deep import has been replaced by a boundary-safe local application of exactly two hash-pinned SQL artifacts without changing their migration or ledger semantics. This correction adds no test, implementation path, package dependency, public export, or production-support surface. This review remains the authoritative S7 inventory: implementation must contain exactly these 20 ordered, direct, literal `it("AW010A-S7 ...")` declarations, with no `skip`, `todo`, `only`, conditional registration, or parameterized registration. A loop inside one registered test is allowed only where that test explicitly names multiple cases below; it does not change the denominator.
...
## Boundary-safe frozen migration revalidation

- The test has exactly one DB cross-package source import, the approved `../../../packages/db/test/support/postgres.js` harness import. It has no `packages/db/src/` import, no `@agent-workspace/db`, `pg`, `@types/pg`, Testcontainers, or migration-runner import, and no dynamic `import(...)`.
- Its two fixed `URL`s name only `0000_aw008_foundation.sql` and `0001_aw010a_channel_stream.sql`. Their independently recomputed SHA-256 values are respectively `645229b04fc4eddd44d47301d47f1efbd394daa6c97852c3ea4a3cbb26df23c2` and `e44f52f786360ac502c0d928cebaebdca718abdd39ae2e78275b9d21505aef26`. Their ledger timestamps, `1787648708709` and `1787695124181`, exactly match the two ordered `meta/_journal.json` entries.
- Independent comparison with pinned `drizzle-orm` 0.45.2 `readMigrationFiles` produced the identical ordered, trimmed nonempty statement sequences: 22 statements for `0000` and 15 for `0001`, split only on `--> statement-breakpoint`. `Promise.all` does not disturb order because its result positions follow the frozen input array.
- Pinned PostgreSQL Drizzle creates schema `drizzle`, then `drizzle.__drizzle_migrations` with exact columns `id SERIAL PRIMARY KEY`, `hash text NOT NULL`, and nullable `created_at bigint`, before opening one transaction for all pending statements and ledger inserts. The local helper reproduces that shape and ordering with one generated migrator client, one `BEGIN`/`COMMIT`, ordered statement execution, an insert after each migration, and rollback on failure. The harness reset drops and recreates both schemas before every application, so the local helper's intentionally exact two-migration bootstrap has the same semantics at this frozen boundary and need not reproduce Drizzle's already-applied lookup path.
- No DB public export was widened; `packages/db/src/index.ts`, both relevant package manifests, and `pnpm-lock.yaml` remain unchanged. The API still has no direct `pg` or `@types/pg` dependency. The migration helper starts no subprocess and performs no dynamic import. The test's sole subprocess call remains the separately required fixed-argument `execFile("docker", ...)` post-stop residue read-back; it is not part of migration application.
- The cumulative checker byte-pins the integration test at `0ca1f534dfd783141cb859e8d92d294ba31242da38697fe020395cebb2e58bd6` and the API Vitest config at `ef19485759b1279fb8430ee36da7ff494e89cc00ee761ec8190eb3bc45fbf030`. It also enforces the sole approved DB import, both URLs, hash verification, statement-breakpoint split, exact ledger tokens, migrator connection, exact ordered 20-test denominator, and the unchanged no-dependency/public-boundary oracles.

## Frozen harness, transaction, and configuration contract

1. `apps/api/test/channel-event-journal.integration.spec.ts` imports only the approved harness source at `../../../packages/db/test/support/postgres.ts`; it does not deep-import DB production source. The test reads the immutable `0000_aw008_foundation.sql` and `0001_aw010a_channel_stream.sql` artifacts by fixed `URL`s, verifies their exact frozen SHA-256 values, splits only on the Drizzle statement breakpoint, and applies both ordered migrations plus the exact Drizzle ledger shape/timestamps with a generated migrator client. Ledger creation occurs before one caller-owned transaction containing both migrations and both ledger inserts, matching pinned Drizzle transaction ownership without widening a DB public export.
2. One digest-pinned harness is started in `beforeAll` with a 180-second hook timeout. `beforeEach` calls `resetDatabase()` and then the local frozen-artifact migration helper. It must leave exactly the two expected ledger rows and may not use a subprocess migration command, a dynamic import, or an unverified SQL artifact. IDs are generated per test from the harness run ID plus an ordinal or `randomUUID`; no tenant, workspace, principal, channel, message, or event identity is reused accidentally.
...
- Targeted S7 integration command: **1/1 file passed, 20/20 tests passed**, with zero skipped/todo tests.
- `pnpm boundaries:check`: **passed**, with zero violations across **97 modules / 229 dependencies** and all boundary-negative fixtures behaving exactly.
- `pnpm scaffold:check`: **passed**, reporting **117 required files, 9 workspace packages, 19 root scripts, and 6 migration tables**.
- Independent frozen-migration parity check: **2/2 hashes**, **2/2 timestamps**, exact **22 + 15** Drizzle statement sequences, exact ledger shape, and one migrator transaction all passed.
- Protected-surface inspection found no DB public-export, package-manifest, lockfile, DB source/migration/harness, or adapter edit from S7 migration correction.

The boundary-safe correction and patched Minimum green therefore close on exactly 20 tests and the same authorized paths and dependencies. No unresolved migration-parity, public-boundary, dependency, migration-subprocess, dynamic-import, teardown, coverage, duplication, transaction-ownership, precision, concurrency, tenant-isolation, privilege-disclosure, or feasibility blocker remains.
